...
Item {
    ...
    Component.onCompleted: {
        if (filter.isNew) {
            ...
            filter.set('color', '#000000')
			filter.set('bgcolor', '#FFFFFF')
            filter.set('font', 'jura')
            filter.set('font-size', 36)
            filter.set('pos-y', 90)
            filter.set('opacity', 65)
    ...
    GridLayout {
        ...
        Preset {
            id: preset
            parameters: ['color', 'gcolor', 'font', 'font-size']
            Layout.columnSpan: 4
        ...
        UndoButton {
            onClicked: opacitySlider.value = 65
        }
